...
#### Cristina Coellen: Sur quoi travaillez-vous en termes d’activité de recherche ? 

Thomas Calvo : Je suis chercheur à l’Institut de recherche stratégique de l’Ecole Militaire et je m’apprête à prendre un poste de chargé de recherche à l’Institut de recherche pour le développement. Donc je passe du Ministère des Armées au Ministère de l’Enseignement supérieur et de la recherche. Ma recherche se concentre principalement sur l’analyse des impacts microéconomiques, donc au niveau des individus et des ménages, des violences sur les décisions et les attitudes des individus. Ça c’est le volet principal. Après j’ai un volet méthodologique sur la mesure de la violence dans la recherche. 

#### CC : En quoi ACLED vous aide dans ces activités de recherche ? 

TC : Ça dépend des travaux que je fais et de ma recherche, mais ACLED est une source de données primaires ou secondaires dans l’analyse des violences que je fais, surtout dans l’analyse des violences liées aux conflits. J’ai plus tendance à utiliser ACLED que d’autres bases de données du même genre, par exemple celle du l’UCDP (Uppsala Conflict Data Program) ou la Global Terrorism Database, parce que c’est moins restrictif en termes de critères d’inclusion. J’utilise beaucoup ACLED pour ce genre de travaux. J’utilise beaucoup ce type de base de données pour ma recherche. 

#### CC: Vous êtes également enseignant. Pour l’enseignement, là aussi ACLED peut vous servir ? 

TC : Oui, alors moi j’enseigne à Sciences Po et à Dauphine, mais c’est surtout à Sciences Po que ça pourrait m’être utile parce que j’enseigne l’économie des conflits et je l’utilise un peu pour des questions de contextualisation, mais pas tant d’ordre méthodologique. Mais beaucoup de papiers que je présente dans ce cours, qui est plutôt un séminaire, utilisent ce type de données. 

#### CC: Dans votre recherche sur la méthodologie de la recherche des conflits, ACLED comme outil de visualisation de bases de données joue-t-il un rôle ? 

TC : Sur la question méthodologique, ce qu’on fait avec une équipe de l’IRD et du CNRS, c’est qu’on essaye d’analyser les réponses à une enquête auprès des ménages, quand il s’agit de reporter l’expérience et les opinions et les attitudes des individus sur des questions d’insécurité. Il y a deux parties là-dessus : la question de l’insécurité liée aux conflits et davantage comme les violences basées sur le genre par exemple. 

#### CC: Vous avez mentionné d’autres bases de données similaires à ACLED. Pourquoi préférez-vous ACLED ? Pensez-vous que c’est le meilleur outil ? 

TC : Non, je ne pense pas que ça soit le meilleur. Je pense que chacune de ces bases de données a ses défauts. Je l’utilise davantage parce qu’il est plus exhaustif dans les critères d’inclusion des événements. Il n’y a pas de critères sur les types des événements inclus, comme c’est le cas pour Global Terrorism Database. Il n’y a pas de critères d’inclusion par rapport au nombre de victimes, comme c’est le cas pour l’UDCP, par exemple. C’est ce que différencie un peu cette base des autres : il y a plus d’événements inclus et cela permet d’aller un peu plus loin dans les analyses, de ne pas seulement s’Intéresser à des événements qui ont récemment couté la vie à des gens, mais aussi des événements moins intenses et moins violents. 

#### CC: ACLED peut-il représenter de manière un peu plus fiable les conflits ou les événements qui tombent sous cette définition dans le monde ? 

TC : C’est difficile à dire parce qu’ACLED, il y a quand même des limites à la couverture d’ACLED. Ça dépend beaucoup de la couverture médiatique dans le pays, il y a des risques que certains journaux soient orientés politiquement, donc ils vont davantage parler de certains événements que d’autres, parce que, comme vous le savez, ACLED est basé sur le recensement des médias d’événements violents. Donc, potentiellement il y a des biais, donc je ne dirais pas que ça soit la meilleure base de données. Par contre, c’est difficile, pour l’instant, de faire beaucoup mieux et malgré les limites qui soient liées à cette base de données, ça reste quelque chose de fiable, même s’il y de l’erreur dans la mesure. 

#### CC : Le fait qu’ACLED se base sur ce qui est rapporté dans les médias, c’est donc un des défauts de la base selon vous ? 

TC : C’est un des défauts de la base. Il y a un problème de couverture médiatique, un problème de temporalité, parce que pour les premiers événements couverts, par exemple en Afrique, qui datent de 1997, on ne peut pas les comparer. C’est difficile de les comparer dans le temps à des événements qui ont eu lieu dans les années 2010 par exemple, parce que la couverture est très différente et c’est difficile de dire qu’en 1997 il y a eu moins d’événements qu’en 2010, juste sur la base de ces données. C’est un problème lié à la couverture. Un des autres défauts de ce type de base de données c’est sur le rapport du nombre de victimes dans ces événements. C’est quelque chose qui est très difficile à vérifier sur le terrain et qui est beaucoup plus difficile à vérifier que si un événement a eu lieu ou pas. ACLED a donc tendance parfois à mettre des valeurs seuils selon le vocabulaire utilisé dans les articles. S’Il y a eu beaucoup de morts, ils vont attribuer un chiffre… pas aléatoire, mais… s’ils disent qu’il y a eu quelques victimes, ils vont en mettre 5, s’ils disent qu’il y en avait beaucoup, ils vont en mettre 10, jusqu’à en mettre 100. Ils vont plutôt mettre 100, mais on ne pourra ni vérifier le nombre de victimes ni donner un ordre de grandeur plutôt qu’un chiffre en valeur absolu. 

#### CC : Dans la recherche, ce nombre de fatalités « aléatoire », cela vous pose problème ? 

TC : Aléatoire ce n’est pas le bon mot ; ce sont des règles pour la base compréhensive avec des règles stables dans le temps. Je ne suis pas très à l’aise avec l’utilisation du nombre des victimes et je l’utilise rarement comme variable d’intérêt principal, parce qu’il y a souvent des erreurs liées à cette variable-là. Je m’intéresse plutôt au nombre d’événements plutôt qu’au nombre de victimes.

#### CC : Pour les étudiants auxquels vous montrez ACLED en cours, pensez-vous que le fonctionnement d’ACLED, par exemple la catégorisation, est-ce quelque chose de clair pour eux ? Ou est-ce quelque chose d’assez complexe ? 

TC : La difficulté c’est plutôt de comprendre les limites de la base d’ACLED et de ne pas extrapoler les résultats qu’on obtient avec ACLED et essayer de comparer ces résultats, qu’on utilise ACLED ou d’autres bases de données que j’ai citées. Je pense que c’est assez intuitif et le matériel qui est fourni par ACLED, les codebooks, sont assez clairs. Après, la difficulté ce n’est pas seulement d’avoir accès à l’Excel ou il y a tous les événements, c’est d’être capable de l’utiliser et de l’utiliser intelligemment. Et cela demande un peu d’expérience et un peu d’apprentissage, mais comme toutes les bases de données, ce n’est pas une spécificité d’ACLED. Par exemple, des événements qui durent sur plusieurs jours, ils peuvent être identifiés plusieurs fois dans la base de données et ce sont des choses où il faut s’entrainer un peu et de s’approprier la base. Ce n’est pas quelque chose d’automatique. 
